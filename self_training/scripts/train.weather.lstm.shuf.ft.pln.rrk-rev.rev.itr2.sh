...
model=lstm
pct=pct-$1
SAVEDIR=self_training/checkpoints/$data/$pct/shuf.ft.pln.rrk-rev.rev.itr2.$model
RESTORECKP=self_training/checkpoints/$data/$pct/shuf.plbl.pln.rrk-rev.rev.itr2.$model/checkpoint_best.pt

mkdir -p $SAVEDIR

fairseq-train self_training/data-prep/$data/$pct/shuf.lbl.rev \
  --user-dir . \
