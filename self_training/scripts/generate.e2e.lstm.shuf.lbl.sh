...
fairseq-generate self_training/data-prep/$data/$pct/$trn \
  ...
  > $SAVEDIR/$gen
bash scripts/measure_scores.e2e.sh $SAVEDIR/$gen self_training/data-prep/$data/$pct/$trn/$testpfx.mr-ar.ar
...
bash scripts/measure_scores.e2e.replfail.sh $SAVEDIR/$gen $base self_training/data-prep/$data/$pct/$trn/$testpfx.mr-ar.ar
