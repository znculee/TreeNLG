...
model=lstm
pct=pct-$1
SAVEDIR=self_training/checkpoints/$data/$pct/shuf.ft.rrk-rev.$model
testpfx=test
...
fairseq-generate self_training/data-prep/$data/$pct/shuf.lbl \
  ...
  > $SAVEDIR/$gen
bash scripts/measure_scores.e2e.sh $SAVEDIR/$gen self_training/data-prep/$data/$pct/shuf.lbl/$testpfx.mr-ar.ar
...
bash scripts/measure_scores.e2e.replfail.sh $SAVEDIR/$gen $base self_training/data-prep/$data/$pct/shuf.lbl/$testpfx.mr-ar.ar
