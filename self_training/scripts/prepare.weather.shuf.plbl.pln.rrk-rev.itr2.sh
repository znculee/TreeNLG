...
pct=pct-$1
dest=$pct/shuf.plbl.pln.rrk-rev.itr2
beam_size=5
...
REVMDLDIR=self_training/checkpoints/$data/$pct/shuf.ft.pln.rrk-rev.rev.$model

mkdir -p $tmp
fairseq-generate $prep/$pct/shuf.lbl.pln \
  --user-dir . \
  --gen-subset train.ulbl \
  --path self_training/checkpoints/$data/$pct/shuf.ft.pln.rrk-rev.$model/checkpoint_best.pt \
  --dataset-impl raw \
  --max-sentences 128 \
